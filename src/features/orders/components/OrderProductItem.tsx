...
import type { OrderProductItemProps } from "@/types/components";
import type { CartAttribute } from "@/types/cart";
...
export default function OrderProductItem({
  ...
  const { data: attributesData } = useProductAttributesQuery(
    showReorder && product ? item.productId : undefined,
  );
  ...
  const { description, cartAttributes } = useMemo(() => {
    ...
    const mappedCartAttrs: CartAttribute[] = [];
    ...
    attributesData.attributes.forEach(attrdef => {
      ...
      if (matchedVal) {
        const attrName = attrdef.displayName ?? attrdef.name ?? "Atributo";
        attrValues.push(`${attrName}: ${matchedVal.value}`);
        mappedCartAttrs.push({ name: attrName, value: matchedVal.value });
      }
    });
