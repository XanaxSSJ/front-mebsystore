...
import type { AddressHttp } from "@/types/user";
import type {
  AddressFormState,
  AddressesListCardProps,
...
const EMPTY_ADDRESSES: AddressHttp[] = [];
...
function ProfilePage() {
  ...
  const { data: provincesData, isLoading: provincesLoading } = useProvincesQuery(
    addressForm.department,
  );

  const { data: districtsData, isLoading: districtsLoading } = useDistrictsQuery(
    addressForm.department,
    addressForm.province,
  );

  const addresses = addressesData ?? EMPTY_ADDRESSES;
  const departments = departmentsData ?? EMPTY_LOCATIONS;
  const provinces = provincesData ?? EMPTY_LOCATIONS;
  const districts = districtsData ?? EMPTY_LOCATIONS;
  ...
  const handleAddressSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Error al guardar dirección"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteAddress = async (addressId: string) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Error al eliminar dirección"));
    } finally {
      setLoading(false);
    }
  };

  const handleEditAddress = (address: AddressHttp) => {
  ...
  return (
    <PageLayout className="w-full pt-12 pb-20">
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-surface mb-2">Mi Perfil</h1>
...
function ProfileSidebar({ activeTab, onSelectProfile, onSelectAddresses }: ProfileSidebarProps) {
  return (
...
function ProfileMessages({ error, success }: ProfileMessagesProps) {
